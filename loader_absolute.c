...
int main(int argc, char **argv)
{
	...
	absolute_loaded_mem_file = fopen(absolute_loaded_mem_file_name, "w");
	if( absolute_loaded_mem_file == NULL)
	{
	...
	while( (r = getline(&line, &len, output_file)) != -1)
	{
		...
		if(tokens != NULL)
		{
			...
			if(!strcmp(tokens[0], "H") && state == 0)
			{
				...
				if(starting_address % 16 != 0)
				{
					for(i = 0; i < 2*(starting_address % 16); i++)
					{
			...
			if(!strcmp(tokens[0], "E"))
			{
				...
				writeMemoryToFile(absolute_loaded_mem_file, memory_blocks, no_of_records);
				state = 2;
			}
	...
	// printf(" line \"%s\"\n", line);
	// printf("state : %d\n", state);
	fprintf(absolute_loaded_mem_file, "\n");
	
	// close files
	fclose(output_file);
	fclose(absolute_loaded_mem_file);
	...
	{
		printf(" > Success!\n");
		printf(" > Program length = %lu bytes\n", (locctr - starting_address + memory_col_n - MAX_ADDRESS_SIZE - 1));
	...
	if(state == 3)
	{
		...
		remove(absolute_loaded_mem_file_name);
	}
	if(line != NULL)
		free(line);
	if(op_tab != NULL)
